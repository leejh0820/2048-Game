            ...
            for (int row = 3; row >= 0; row --) {
                ...
                if (tile == null) {
                    for (int row1 = row-1; row1 >= 0; row1 --) {
                        Tile tile1 = _board.tile(col, row1);
                        if (tile1 != null) {
                            _board.move(col, row, tile1);
                            row ++;
                            break;
                        }
                    }
                    ...
                    for (int row2 = row -1; row2 >= 0; row2 --) {
                        Tile tile2 = _board.tile(col, row2);
                        if (tile2 != null && tile.value() == tile2.value()) {
                            _board.move(col, row, tile2);
                            this._score += tile.value()*2;
                            break;
                        } else if (tile2 != null && tile.value() != tile2.value()) {
                            _board.move(col, row-1, tile2);
                            break;
                        }
                    }
                }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int col = 0; col < size; col += 1) {
            for (int row = 0; row < size; row += 1) {
                ...
                Tile tile1 = b.tile(col + 1, row);
                if (tile.value() == tile1.value()){
                    return true;
                }
            }
            for (int col1 = 0; col1 < size; col1 += 1) {
                for (int row1 = 0; row1 < size; row1 += 1) {
                    Tile tile2 = b.tile(col1, row1);
                    if (row1 == 3) {
                        break;
                    }
                    Tile tile3 = b.tile(col1, row1 + 1);
